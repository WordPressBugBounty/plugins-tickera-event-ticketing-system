...
= 3.6.0.1 =
Removes the "Powered by Tickera" credit from tickets.
...
* Improved Delete Info batch processing with permanent ticket deletion, validation, scoped progress indicators, and reliable AJAX error recovery.
* Resolved a CSS conflict between Tickera and the Divi Theme Customizer.
* Fixed barcode size not reflecting correctly in generated PDF tickets.
* Ability to `Remove Check-ins History` in Tickera > Settings > Delete Info section.
* Unified check-in and checkout history under `tc_checkins`, using a new `direction` field with `in` and `out` values.
* Added automatic migration and removal of legacy `tc_checkouts` metadata.
* Improved attendance history ordering, validation, reporting, and Check-in Details information.
* Predefined custom capabilities for Tickera > Settings tabs (e.g manage_general_settings_cap, manage_api_settings_cap, manage_gateways_settings_cap, manage_email_settings_cap )
* Patchstack: Security patch for PHP Object Injection Vulnerability.

= 3.6.0.2 22/07/2026 =
* Unable to "Cancel Pending Orders" scheduled task. [Fixed]
* Improved Database Query Performance.
* Wordfence vulnerability patch.

= 3.6.0.1 13/07/2026 =
* Removed the "Powered by Tickera" credit from tickets.
* Fixed: Instantiate TC_Shortcode_Builder class after the plugin is fully loaded.
* Support "Users > Ticket Order History" for Bridge for Woocommerce.
* Wordfence vulnerability patch.
* Additional WP 7.0 Compliance.
...
= 3.5.7.1 12/05/2026 =
* Added support for the <p> tag in the "Event Terms & Conditions" Ticket Template element.
* Adjusted the session handler to execute within Tickera core pages only.
...
* Missing event's options in Divi > Text Element > Tickera Shortcode Builder. [Fixed]
* Applied epsilon-based (1e-10) threshold when comparing two floating point numbers. This is to avoid floating-point precision errors for values close to zero.
* Attendees Information label persistently appears in the cart/checkout page regardless "Show attendee fields" setting. [Fixed]
* Clearing out PHP8.5 deprecated errors.
