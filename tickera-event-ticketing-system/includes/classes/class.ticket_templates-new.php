		...
		function generate_preview( $ticket_instance_id = false, $force_download = false, $template_id = false, $ticket_type_id = false, $string_attachment = false ) {
			...
			if ( $ticket_instance_id ) {
				...
				if ( 'publish' == $ticket_instance_status ) {
                    ...
                    $post_id = (!$ticket_template ) ? get_post_meta( $ticket_instance_ticket_type_id, tickera_apply_filters( 'tickera_ticket_template_field_name', '_ticket_template', $ticket_instance_ticket_type_id ), true ) : $ticket_template;
				} else {
			...
			$pdf = new \Tickera\TCPDF( $tc_document_orientation, TC_PDF_UNIT, tickera_apply_filters( 'tickera_additional_ticket_document_size_output', $tc_document_paper_size ), true, tickera_apply_filters( 'tickera_ticket_document_encoding', 'UTF-8' ), false );
			...
			$ticket_template_auto_pagebreak	 = ( isset( $tickera_general_settings[ 'ticket_template_auto_pagebreak' ] ) && 'yes' == $tickera_general_settings[ 'ticket_template_auto_pagebreak' ] ) ? true : false;
			...
			if ( $string_attachment ) {
				...
				if ( true == tickera_apply_filters( 'tickera_exit_after_pdf_output', true ) )
					exit;
			}
		}
