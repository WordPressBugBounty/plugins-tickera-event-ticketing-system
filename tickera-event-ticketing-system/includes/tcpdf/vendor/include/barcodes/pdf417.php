...
// definitions
if (!defined('TC_PDF417DEFS')) {

	/**
	 * Indicate that definitions for this class are set
	 */
	define('TC_PDF417DEFS', true);
...
class PDF417 {
	...
	public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
		...
		$nce = ($numcw + $errsize + 1);
		$cols = round((sqrt(4761 + (68 * $aspectratio * TC_ROWHEIGHT * $nce)) - 69) / 34);
		// adjust cols
		if ($cols < 1) {
			$cols = 1;
		} elseif ($cols > 30) {
		...
		$pstart = str_repeat('0', TC_QUIETH).$this->start_pattern;
		$pstop = $this->stop_pattern.str_repeat('0', TC_QUIETH);
		$barcode_array['num_rows'] = ($rows * TC_ROWHEIGHT) + (2 * TC_QUIETV);
		$barcode_array['num_cols'] = (($cols + 2) * 17) + 35 + (2 * TC_QUIETH);
		$barcode_array['bcode'] = array();
		// build rows for vertical quiet zone
		if (TC_QUIETV > 0) {
			$empty_row = array_fill(0, $barcode_array['num_cols'], 0);
			for ($i = 0; $i < TC_QUIETV; ++$i) {
