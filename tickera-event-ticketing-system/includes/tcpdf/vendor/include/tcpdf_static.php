...
class TCPDF_STATIC {
    ...
    public static function set_mqr($mqr) {
        if (!defined('TC_PHP_VERSION_ID')) {
            $version = PHP_VERSION;
            define('TC_PHP_VERSION_ID', (($version[0] * 10000) + ($version[2] * 100) + $version[4]));
        }
    ...
    public static function getObjFilename($type='tmp', $file_id='') {
        return tempnam(TC_K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.md5(TCPDF_STATIC::getRandomSeed()).'_');
    }
    ...
    public static function fixHTMLCode($html, $default_css, $tagvs, $tidy_options, &$tagvspaces) {
         ...
         */
        $default_css = $default_css ? $default_css : '';
        $tagvs = $tagvs ? $tagvs : '';
        $tidy_options = $tidy_options ? $tidy_options : '';
    ...
    public static function pregSplit($pattern, $modifiers, $subject, $limit=NULL, $flags=NULL) {
        ...
        if ((strpos($modifiers, 'u') === FALSE) OR (count(preg_split('//u', "\n\t", -1, PREG_SPLIT_NO_EMPTY)) == 2)) {
            ...
            if ( TCPDF_STATIC::is_php_gt_eight_zero() ) {
                $limit = is_null( $limit ) ? -1 : $limit;
                $flags = is_null( $flags ) ? 0 : $flags;
            }
     ...
     * @return array|void array of page dimensions.
     * @since 5.0.010 (2010-05-17)
     * @public static
     */
    ...
    /**
     * Tickera - Check if PHP Version is above 8.0
     * @return bool
     * @since 3.5.1.6
     */
    public static function is_php_gt_eight_zero() {
        $version = phpversion();
        return ( version_compare( $version, '8.0', '>' ) ) ? true : false;
    }
