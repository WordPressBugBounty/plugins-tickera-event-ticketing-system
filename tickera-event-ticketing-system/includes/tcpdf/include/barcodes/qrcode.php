...
// definitions
if (!defined('TC_QRCODEDEFS')) {

	/**
	 * Indicate that definitions for this class are set
	 */
	define('TC_QRCODEDEFS', true);
	 ...
	 * Encoding mode
	 */
	define('TC_QR_MODE_NL', -1);

	/**
	 * Encoding mode numeric (0-9). 3 characters are encoded to 10bit length. In theory, 7089 characters or less can be stored in a QRcode.
	 */
	define('TC_QR_MODE_NM', 0);

	/**
	 * Encoding mode alphanumeric (0-9A-Z $%*+-./:) 45characters. 2 characters are encoded to 11bit length. In theory, 4296 characters or less can be stored in a QRcode.
	 */
	define('TC_QR_MODE_AN', 1);

	/**
	 * Encoding mode 8bit byte data. In theory, 2953 characters or less can be stored in a QRcode.
	 */
	define('TC_QR_MODE_8B', 2);

	/**
	 * Encoding mode KANJI. A KANJI character (multibyte character) is encoded to 13bit length. In theory, 1817 characters or less can be stored in a QRcode.
	 */
	define('TC_QR_MODE_KJ', 3);

	/**
	 * Encoding mode STRUCTURED (currently unsupported)
	 */
	define('TC_QR_MODE_ST', 4);
	 ...
	 */
	define('TC_QR_ECLEVEL_L', 0);

	/**
	 * Error correction level M : About 15% or less errors can be corrected.
	 */
	define('TC_QR_ECLEVEL_M', 1);

	/**
	 * Error correction level Q : About 25% or less errors can be corrected.
	 */
	define('TC_QR_ECLEVEL_Q', 2);

	/**
	 * Error correction level H : About 30% or less errors can be corrected.
	 */
	define('TC_QR_ECLEVEL_H', 3);
	 ...
	 */
	define('TC_QRSPEC_VERSION_MAX', 40);

	/**
	 * Maximum matrix size for maximum version (version 40 is 177*177 matrix).
	 */
    define('TC_QRSPEC_WIDTH_MAX', 177);
	 ...
	 */
    define('TC_QRCAP_WIDTH',    0);

    /**
	 * Matrix index to get number of words from $capacity array.
	 */
    define('TC_QRCAP_WORDS',    1);

    /**
	 * Matrix index to get remainder from $capacity array.
	 */
    define('TC_QRCAP_REMINDER', 2);

    /**
	 * Matrix index to get error correction level from $capacity array.
	 */
    define('TC_QRCAP_EC',       3);
	 ...
	 */
    define('TC_STRUCTURE_HEADER_BITS',  20);

    /**
	 * Max number of symbols for structured mode
	 */
    define('TC_MAX_STRUCTURED_SYMBOLS', 16);
	 ...
	 */
	define('TC_QR_FIND_BEST_MASK', true);

	/**
	 * if false, checks all masks available, otherwise value tells count of masks need to be checked, mask id are got randomly
	 */
	define('TC_QR_FIND_FROM_RANDOM', 2);

	/**
	 * when TC_QR_FIND_BEST_MASK === false
	 */
	define('TC_QR_DEFAULT_MASK', 2);
...
class TCPDF_QRcode{ 
	 ...
	 * @protected
	 */
	protected $level = TC_QR_ECLEVEL_L;

	/**
	 * Encoding mode.
	 * @protected
	 */
	protected $hint = TC_QR_MODE_8B;
	...
	public function __construct($code, $eclevel = 'L') {
		...
		if ($this->level === false) {
			$this->level = TC_QR_ECLEVEL_L;
		}
		if (($this->hint != TC_QR_MODE_8B) AND ($this->hint != TC_QR_MODE_KJ)) {
			return false;
		}
		if (($this->version < 0) OR ($this->version > TC_QRSPEC_VERSION_MAX)) {
			return false;
		}
		$this->items = array();
	...
	protected function encodeMask($mask) {
		...
		// masking
		$this->runLength = array_fill(0, TC_QRSPEC_WIDTH_MAX + 1, 0);
		if ($mask < 0) {
			if (TC_QR_FIND_BEST_MASK) {
				$masked = $this->mask($this->width, $this->frame, $this->level);
			} else {
				$masked = $this->makeMask($this->width, $this->frame, (intval(TC_QR_DEFAULT_MASK) % 8), $this->level);
			}
		} else {
	 ...
	 protected function calcN1N3($length) {
		...
		for ($i=0; $i<$length; ++$i) {
			if ($this->runLength[$i] >= 5) {
				$demerit += (TC_N1 + ($this->runLength[$i] - 5));
			}
			if ($i & 1) {
	 ...
	 */
	 protected function mask($width, $frame, $level) {
		...
		if (TC_QR_FIND_FROM_RANDOM !== false) {
			$howManuOut = 8 - (TC_QR_FIND_FROM_RANDOM % 9);
		...
		foreach ($checked_masks as $i) {
			...
			$demerit = (int)((int)(abs($blacks - 50) / 5) * TC_N4);
	 ...
	 */
	 protected function identifyMode($pos) {
		if ($pos >= strlen($this->dataStr)) {
			return TC_QR_MODE_NL;
		}
		$c = $this->dataStr[$pos];
		if ($this->isdigitat($this->dataStr, $pos)) {
			return TC_QR_MODE_NM;
		} elseif ($this->isalnumat($this->dataStr, $pos)) {
			return TC_QR_MODE_AN;
		} elseif ($this->hint == TC_QR_MODE_KJ) {
			if ($pos+1 < strlen($this->dataStr)) {
				$d = $this->dataStr[$pos+1];
				$word = (ord($c) << 8) | ord($d);
				if (($word >= 0x8140 && $word <= 0x9ffc) OR ($word >= 0xe040 && $word <= 0xebbf)) {
					return TC_QR_MODE_KJ;
				}
			}
		}
		return TC_QR_MODE_8B;
	}
	 ...
	 */
	 protected function eatNum() {
		$ln = $this->lengthIndicator(TC_QR_MODE_NM, $this->version);
		$p = 0;
		...
		$this->items = $this->appendNewInputItem($this->items, TC_QR_MODE_NM, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function eatAn() {
		$la = $this->lengthIndicator(TC_QR_MODE_AN,  $this->version);
		$ln = $this->lengthIndicator(TC_QR_MODE_NM, $this->version);
		$p =1 ;
		...
		$this->items = $this->appendNewInputItem($this->items, TC_QR_MODE_AN, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function eatKanji() {
		$p = 0;
		while($this->identifyMode($p) == TC_QR_MODE_KJ) {
			$p += 2;
		}
		$this->items = $this->appendNewInputItem($this->items, TC_QR_MODE_KJ, $p, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function eat8() {
		...
		while($p < $dataStrLen) {
			$mode = $this->identifyMode($p);
			if ($mode == TC_QR_MODE_KJ) {
				break;
			}
			if ($mode == TC_QR_MODE_NM) {
				$q = $p;
				while($this->isdigitat($this->dataStr, $q)) {
					$q++;
				}
		...
		$run = $p;
		$this->items = $this->appendNewInputItem($this->items, TC_QR_MODE_8B, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function encodeModeNum($inputitem, $version) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(TC_QR_MODE_NM, $version), $inputitem['size']);
		for ($i=0; $i < $words; ++$i) {
	 ...
	 */
	 protected function encodeModeAn($inputitem, $version) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(TC_QR_MODE_AN, $version), $inputitem['size']);
		for ($i=0; $i < $words; ++$i) {
	 ...
	 */
	 protected function encodeMode8($inputitem, $version) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(TC_QR_MODE_8B, $version), $inputitem['size']);
	 ...
	 */
	 protected function encodeModeKanji($inputitem, $version) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(TC_QR_MODE_KJ, $version), (int)($inputitem['size'] / 2));
	 ...
	 */
	 protected function insertStructuredAppendHeader($items, $size, $index, $parity) {
		if ($size > TC_MAX_STRUCTURED_SYMBOLS) {
			return -1;
		}
		if (($index <= 0) OR ($index > TC_MAX_STRUCTURED_SYMBOLS)) {
			return -1;
		}
		$buf = array($size, $index, $parity);
		$entry = $this->newInputItem(TC_QR_MODE_ST, 3, buf);
	 ...
	 protected function estimateBitStreamSize($items, $version) {
		...
		foreach ($items as $item) {
			switch($item['mode']) {
				...
				case TC_QR_MODE_ST: {
					return TC_STRUCTURE_HEADER_BITS;
				}
				default: {
					return 0;
				}
	...
	protected function getDataLength($version, $level) {
		return $this->capacity[$version][TC_QRCAP_WORDS] - $this->capacity[$version][TC_QRCAP_EC][$level];
	}
	...
	protected function getECCLength($version, $level){
		return $this->capacity[$version][TC_QRCAP_EC][$level];
	}
	...
	protected function getWidth($version) {
		return $this->capacity[$version][TC_QRCAP_WIDTH];
	}
	...
	protected function getRemainder($version) {
		return $this->capacity[$version][TC_QRCAP_REMINDER];
	}
	...
	protected function getMinimumVersion($size, $level) {
		for ($i = 1; $i <= TC_QRSPEC_VERSION_MAX; ++$i) {
			$words = ($this->capacity[$i][TC_QRCAP_WORDS] - $this->capacity[$i][TC_QRCAP_EC][$level]);
			if ($words >= $size) {
				return $i;
			}
		}
	...
	protected function getVersionPattern($version) {
		if (($version < 7) OR ($version > TC_QRSPEC_VERSION_MAX)) {
			return 0;
		}
	...
	protected function createFrame($version) {
		$width = $this->capacity[$version][TC_QRCAP_WIDTH];
	...
	protected function newFrame($version) {
		if (($version < 1) OR ($version > TC_QRSPEC_VERSION_MAX)) {
			return NULL;
		}
