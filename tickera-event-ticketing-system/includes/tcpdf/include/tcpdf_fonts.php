...
class TCPDF_FONTS {
    ...
    public static function _getfontpath() {
        if (!defined('TC_K_PATH_FONTS') AND is_dir($fdir = realpath(dirname(__FILE__).'/../fonts'))) {
            if (substr($fdir, -1) != '/') {
                $fdir .= '/';
            }
            define('TC_K_PATH_FONTS', $fdir);
        }
        return defined('TC_K_PATH_FONTS') ? TC_K_PATH_FONTS : '';
    }
...
// ====================================================================================================================
    ...
    public static function UTF8ArrayToUniArray($ta, $isunicode=true) {
        if ($isunicode) {
            return array_map(array('Tickera\TCPDF_FONTS', 'unichrUnicode'), $ta);
        }
        return array_map(array('Tickera\TCPDF_FONTS', 'unichrASCII'), $ta);
    }
    ...
    public static function UTF8StringToArray($str, $isunicode, &$currentfont) {
         ...
         */
        $isunicode = !$isunicode ? $isunicode : true;

        if ($isunicode) {
            // requires PCRE unicode support turned on
            $chars = TCPDF_STATIC::pregSplit('//','u', $str, -1, PREG_SPLIT_NO_EMPTY);
            $carr = array_map(array('Tickera\TCPDF_FONTS', 'uniord'), $chars);
        } else {
            $chars = str_split($str);
    ...
    public static function utf8StrRev($str, $setbom, $forcertl, $isunicode, &$currentfont) {
         ...
         */
        $setbom = $setbom ? $setbom : false;
        $forcertl = $forcertl ? $forcertl : false;
        $isunicode = !$isunicode ? $isunicode : true;
    ...
    public static function utf8Bidi($ta, $str, $forcertl, $isunicode, &$currentfont) {
        ...
        // TC_N1. A sequence of neutrals takes the direction of the surrounding strong text if the text on both sides has the same direction. European and Arabic numbers act as if they were R in terms of their influence on neutrals. Start-of-level-run (sor) and end-of-level-run (eor) are used at level run boundaries.
        $prevlevel = -1;
        $levcount = 0;
        for ($i=0; $i < $numchars; ++$i) {
            if (($levcount > 0) AND (($i+1) < $numchars) AND ($chardata[($i+1)]['level'] == $prevlevel)) {
                if (($chardata[$i]['type'] == 'N') AND ($chardata[($i-1)]['type'] == 'L') AND ($chardata[($i+1)]['type'] == 'L')) {
                    ...
                    // TC_N2. Any remaining neutrals take the embedding direction
                    $chardata[$i]['type'] = $chardata[$i]['sor'];
                }
